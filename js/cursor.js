    ...
    document.onmousemove = function(t) {
        if (sparkles_enabled && !t.buttons) {
            const e = Math.sqrt(Math.pow(t.movementX, 2) + Math.pow(t.movementY, 2))
              , i = 30 * t.movementX * 2 / e
              , n = 30 * t.movementY * 2 / e
              , s = e / 30;
            let l = 0
              , r = t.pageY
              , a = t.pageX;
...
function create_star(t, e, i=1) {
    ...
    s >= 0 ? (star_remaining_ticks[s] = 60,
    star_x[s] = t,
    star[s].style.left = t + "px",
    star_y[s] = e,
    star[s].style.top = e + "px",
    star[s].style.clip = "rect(0px, 5px, 5px, 0px)",
    star[s].childNodes[0].style.backgroundColor = star[s].childNodes[1].style.backgroundColor = "#ffff ",
...
function update_star(t) {
    ...
    !1) : (30 === star_remaining_ticks[t] && (star[t].style.clip = "rect(1px, 4px, 4px, 1px)"),
...
function update_tiny(t) {
    ...
    30 === tiny_remaining_ticks[t] && (tiny[t].style.width = "1px",
